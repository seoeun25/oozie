...
public class TestCoordActionNotificationXCommand extends XDataTestCase {
    ...
    public void setUp() throws Exception {
        ...
        LocalOozie.start("/hang/*", HangServlet.class, "/notification/*", NotificationServlet.class);
    }
    ...
    public void testQueueNotificationCommand() throws Exception {
        ...
        jobConfiguration.set(OozieClient.COORD_ACTION_NOTIFICATION_URL,
                LocalOozie.getServletURL("/notification") + "/coord?actionId=$actionId&status=$status");
        _testQueueNotificationCommand(1);
    }

    public void testSkipNotificationCommand() throws Exception {
        NotificationServlet.reset();
        _testQueueNotificationCommand(0);
    }

    private void _testQueueNotificationCommand(int expectedQueuingCommand) throws Exception{
        ...
        Assert.assertEquals(expectedQueuingCommand, callableQueueService.getQueueDump().size());
        // wait until all commands are executed.
        waitFor(3 * 1000, new Predicate() {
            @Override
            public boolean evaluate() throws Exception {
                return callableQueueService.getQueueDump().size() == 0;
            }
        });

        Assert.assertEquals(expectedQueuingCommand, NotificationServlet.history.size());
