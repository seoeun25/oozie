...
public class TestWorkflowNotificationXCommand extends XDataTestCase {
    ...
    public void setUp() throws Exception {
        ...
        LocalOozie.start(oozieConf, new String[]{PurgeService.class.getCanonicalName()},
                "/hang/*", HangServlet.class, "/notification/*", CallbackServlet.class);
    ...
    /**
     * Test : The job notification command are queued when WORKFLOW_NOTIFICATION_URL is configured.
     * And, the action notification command are queued when ACTION_NOTIFICATION_URL is configured.
     * @throws Exception
     */
    public void testQueueNotificationCommand1() throws Exception {
        ...
        String jobId = submit(); // 2 is for CompositeCallable
        long queued = instrumentaionOfQueued();
        assertEquals(5 + 2, queued);  // 2 is for CompositeCallable
    ...
    /**
     * Test : The action notification command are queued when ACTION_NOTIFICATION_URL is configured.
     * @throws Exception
     */
    public void testQueueNotificationCommand3() throws Exception {
        ...
        String jobId = submit();
        long queued = instrumentaionOfQueued();
        assertEquals(3 + 1, queued);  // 1 is for CompositeCallable
        for (String notification : CallbackServlet.history) {
            System.out.println("notification history = " + notification);
        }
        assertEquals(2, CallbackServlet.history.size());
    ...
    /**
     * Test : Skip queueing notification command when the notification url are not configured.
     * @throws Exception
     */
